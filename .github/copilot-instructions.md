...
## Project Context
- **Stack**: TypeScript (ESM), MCP SDK (`@modelcontextprotocol/sdk`), `node-freshdesk` client library
...
- Destructive tools (deletes, merge, soft-delete, make-agent) must carry the ⚠ warning prefix and `destructiveHint` annotation, and confirm via elicitation
- Resource cleanup: HTTP server must handle request close gracefully
...
- Use the `node-freshdesk` client library for API calls, not raw `fetch`
- Error responses from Freshdesk APIs should be mapped to meaningful MCP error messages

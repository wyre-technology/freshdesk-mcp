...
function startHttpServer(): void {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    const domain = isGatewayMode ? (req.headers['x-freshdesk-domain'] as string | undefined) : undefined;
    const apiKey = isGatewayMode ? (req.headers['x-freshdesk-api-key'] as string | undefined) : undefined;

    if (domain && apiKey) {
      await runWithCredentials({ domain, apiKey }, handle);
    } else {
      await handle();
    }
  });
